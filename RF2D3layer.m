...
% Run multiple attentional states, with same spike trains (s1) from layer 2
% Different attentional states corresponds to different param(2).Iapp and param(2).Jx.  
% The number of parameter sets for Layer 3 is the column number of
% param(2).Iapp and param(2).Jx. 
% size(Jx,2) should = size(testp.Iapp,2);

% RF2D3layer(option, ParamChange)

% param is a struct w/ fields: Ne, Ni, Nx, Jx, Jr, Kx, Kr,
...
%     save(filename,'T')
    ...
    if option.Layer1only==0
        ...
        fprintf('\nTotal number of parameter sets to run: %d\n',Np)
        if option.fixW
            if option.useWfile==1
